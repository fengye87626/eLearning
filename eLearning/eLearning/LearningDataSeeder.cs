...
namespace Learning.Data
{
    ...
    /// 1.In the constructor of the class we set the property AutomaticMigrationsEnabled to true which means 
    /// that we need EF to handle the automatic migration for us without caring about DB Versioning. 
    /// As well we set the property AutomaticMigrationDataLossAllowed  to true, this is dangerous to set in production environment. 
    /// If it was set to false an exception will be thrown if data loss may occur as part of an automatic migration, but for our series it is fine to keep it to true.
